...
impl<'de> serde::Deserializer<'de> for ValueDeserializer<'de> {
        ...
        V: serde::de::Visitor<'de>,
    {
        ...
        match &self.0.value_type {
            ...
            // FIXME
            Some(ValueType::PipelineValue(_)) => Err(Error::from(crate::error::ErrorCode::Custom(
                "PipelineValue is not supported".to_string(),
            ))),
        }
    }
...
struct NewtypeStructMapAccess<'a> {
    name: &'static str,
    value: Option<&'a str>,
    index: usize,
}
...
struct FunctionValueMapAccess<'a> {
    function: &'a crate::google::firestore::v1::Function,
    index: usize,
}

impl<'a> FunctionValueMapAccess<'a> {
    fn new(function: &'a crate::google::firestore::v1::Function) -> Self {
...
struct ValueMapDeserializerAccess<'a> {
